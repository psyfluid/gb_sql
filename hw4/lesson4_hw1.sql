...
# 2. Вывести магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
SELECT shopname
FROM shops
         JOIN cats ON shops.id = cats.shops_id
...
FROM shops
         JOIN (SELECT shops_id
               FROM cats
               WHERE cats.name = 'Murzik') c ON shops.id = c.shops_id;
...
SELECT shopname
FROM shops
WHERE EXISTS(SELECT shops_id FROM cats WHERE name = 'Murzik' AND shops.id = cats.shops_id);


# 3. Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT shopname
FROM shops
WHERE id NOT IN (SELECT cats.shops_id FROM cats WHERE name IN ('Murzik', 'Zuza'));
...
# 12 февраля не должно быть включено в результат, поскольку это будет уже 8 дней,
# поэтому правильный вариант получения продаж за неделю — это с начала дня 05.02 по начало дня 12.02.
SELECT DISTINCT an_name,
                an_price
FROM Analysis
         JOIN (SELECT ord_an
               FROM Orders
               WHERE ord_datetime BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', INTERVAL 1 WEEK)) O
              ON Analysis.an_id = O.ord_an
ORDER BY an_name;
